...
if ( ! class_exists( 'MUCD_Files' ) ) {
	...
	/**
	 * Recursive copy of the uploads directory between sites, used when duplicating a site.
	 */
	class MUCD_Files {
		...
		public static function copy_files( $from_site_id, $to_site_id ) {
			...
			$from_dir['exclude'] = ( (int) MUCD_PRIMARY_SITE_ID === (int) $from_site_id ) ? MUCD_Option::get_primary_dir_exclude() : array();
		...
		public static function recurse_copy( $src, $dst, $exclude_dirs = array() ) {
			...
			$dst = rtrim( $dst, '/' );
			$dir = opendir( $src );
			// phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.file_system_operations_mkdir -- local disk copy between upload dirs; $dst may already exist, that's expected.
			@mkdir( $dst );
			// phpcs:ignore Generic.CodeAnalysis.AssignmentInCondition.FoundInWhileCondition -- standard readdir() iteration idiom.
		...
		public static function init_dir( $path ) {
			if ( ! file_exists( $path ) ) {
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_mkdir -- local disk operation on the uploads dir.
				return mkdir( $path, 0755 );
			} elseif ( is_dir( $path ) ) {
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- local disk operation on the uploads dir.
				if ( ! is_writable( $path ) ) {
					// 0755, not 0777: avoid making the directory world-writable.
					// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_chmod -- local disk operation on the uploads dir.
				...
				foreach ( $objects as $object ) {
					if ( '.' !== $object && '..' !== $object ) {
						if ( 'dir' === filetype( $dir . '/' . $object ) ) {
							...
							// phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink -- local disk operation on the uploads dir.
							unlink( $dir . '/' . $object );
						}
					}
				}
				reset( $objects );
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_rmdir -- local disk operation on the uploads dir.
				rmdir( $dir );
			}
		}
		 ...
		 * @param  int    $to_site_id the new site id to roll back
		 */
