...
if ( ! class_exists( 'MUCD_Duplicate' ) ) {
	...
	/**
	 * Orchestrates the full site duplication process: admin user, blog, data, files and users.
	 */
	class MUCD_Duplicate {

		/**
		 * The MUCD_Log instance used for the current duplication run, or false when logging is off.
		 *
		 * @var MUCD_Log|false
		 */
		public static $log;
		...
		public static function duplicate_site( $data ) {
			...
			$user_id = self::create_admin( $email, $domain );
		...
		public static function copy_users( $from_site_id, $to_site_id ) {
			...
			if ( is_main_site( $from_site_id ) ) {
				...
				if ( ! empty( $all_sites_ids ) ) {
					$all_sites_ids = array_column( $all_sites_ids, 0 );
				}
			} else {
			...
			foreach ( $users as $user ) {
				if ( $user->user_email !== $admin_email ) {
					...
					$all_meta = array_column( get_user_meta( $user->ID ), 0 );
					...
					foreach ( $all_meta as $metakey => $metavalue ) {
						...
						if ( $prefix === $from_site_prefix ) {
							...
							if ( $is_from_main_site ) {
								...
								if ( count( $parts ) > 1 && in_array( (int) $parts[0], $all_sites_ids, true ) ) {
									continue;
								}
							}
